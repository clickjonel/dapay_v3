<script setup>
    ...
    function submit() {
        ...
        form.put(`/programs/${props.program.id}/update`, {
            onStart: () => {
                showLoader();
            },
            onSuccess: () => {
...
<template>
    <main class="w-full h-full flex flex-col justify-start items-start gap-4 p-4">
        ...
        <section class="w-full flex flex-col gap-3 pb-3 border-b border-stone-200">
            <span class="text-lg font-bold uppercase tracking-tight text-stone-900">Edit Program</span>
        </section>
        ...
        <section class="w-full flex flex-col justify-start items-start gap-6">
            <div class="w-full flex flex-col justify-start items-start border border-stone-200">
                <div class="w-full flex bg-[#3F4E2E] text-white p-2">
                    <span class="text-[13px] font-semibold uppercase tracking-wide">Program Details</span>
                </div>

                <div class="w-full flex flex-col justify-start items-start gap-4 p-4">
                    <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="w-full flex flex-col gap-1.5">
                            <label for="program_name" class="text-[13px] font-medium text-stone-600">Program Name</label>
                            <input
                                id="program_name"
                                v-model="form.name"
                                type="text"
                        ...
                        <div class="w-full flex flex-col gap-1.5">
                            <label for="program_status" class="text-[13px] font-medium text-stone-600">Status</label>
                            <select
                                id="program_status"
                                v-model="form.active"
                                class="w-full h-10 px-3 border border-stone-300 text-sm outline-none focus:border-[#3F4E2E] transition-colors"
